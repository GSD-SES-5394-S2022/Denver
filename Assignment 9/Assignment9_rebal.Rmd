...
```{r}
	...
	write_csv(total_time, here("Assignment10",
                         "total_time.csv"))
```
...
Writing updated existing and alt skims to file

```{r}
write.csv(skims, here("existing",
                                "data",
                                "ex_skims_update.csv"))

write.csv(alt_skims, here("alternative",
                                "data",
                                "alt_skims_update.csv"))
```
...
```{r}
hbo <- c(mode_by_purpose[[1,2]],mode_by_purpose[[1,3]],mode_by_purpose[[1,4]],mode_by_purpose[[1,5]],mode_by_purpose[[1,6]])
mode_types <- c("Bike", "SOV", "HOV", "Transit","Walk")
hbodf <- data.frame(hbo, mode_types)

hbo_bar_chart <- ggplot(hbodf, aes(fill=mode_types, 
                   x=1, 
                   y=hbo,
                   label = paste0(round(hbo*100), "%"))) +
...
```
```{r}
pie_chart_hbo <- hbo_bar_chart +
  coord_polar(theta = "y") +
  theme_void()

pie_chart_hbo 

ggsave("hbo_mode_choice.pdf", pie_chart_hbo)
```
```{r}
hbw <- c(mode_by_purpose[[2,2]],mode_by_purpose[[2,3]],mode_by_purpose[[2,4]],mode_by_purpose[[2,5]],mode_by_purpose[[2,6]])
mode_types <- c("Bike", "SOV", "HOV", "Transit","Walk")
hbwdf <- data.frame(hbw, mode_types)

hbw_bar_chart <- ggplot(hbwdf, aes(fill=mode_types, 
                   x=1, 
                   y=hbw,
                   label = paste0(round(hbo*100), "%"))) +
    geom_bar(stat="identity")  +
...
```{r}
pie_chart_hbw <- hbw_bar_chart +
  coord_polar(theta = "y") +
  theme_void()

pie_chart_hbw

ggsave("hbw_mode_choice.pdf", pie_chart_hbw)
```
```{r}
nhb <- c(mode_by_purpose[[3,2]],mode_by_purpose[[3,3]],mode_by_purpose[[3,4]],mode_by_purpose[[3,5]],mode_by_purpose[[3,6]])
mode_types <- c("Bike", "SOV", "HOV", "Transit","Walk")
nhbdf <- data.frame(nhb, mode_types)

nhb_bar_chart <- ggplot(nhbdf, aes(fill=mode_types, 
                   x=1, 
                   y=nhb,
                   label = paste0(round(hbo*100), "%"))) +
    geom_bar(stat="identity")  +
...
```{r}
pie_chart_nhb <- nhb_bar_chart +
  coord_polar(theta = "y") +
  theme_void()

pie_chart_nhb

ggsave("nhb_mode_choice.pdf", pie_chart_nhb)
```